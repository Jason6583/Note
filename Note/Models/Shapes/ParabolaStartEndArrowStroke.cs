...
namespace Note.Models
{
    ...
    /// 抛物线两端都带箭头
    /// </summary>
    public class ParabolaStartEndArrowStroke : ShapeStroke
    {
        protected override Path GetShape()
        {
            var points = GetPoints();

            var startArrowPath = GetArrow(points[2], points[1]);
            var endArrowPath = GetArrow(points[points.Count - 3], points[points.Count - 2]);
